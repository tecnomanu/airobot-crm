...
class ClientDispatchController extends Controller
{
     ...
     */
    public function dispatch(SendLeadToClientRequest $request, string $clientId, string $leadId): JsonResponse
    {
        try {
     ...
     */
    public function status(string $clientId, string $leadId): JsonResponse
    {
        try {
            $lead = $this->leadService->getLeadById($leadId);
