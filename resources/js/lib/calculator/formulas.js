/**
 * Sistema de evaluación de fórmulas para Calculator
...
/**
 * Evalúa una fórmula de Calculator
...
/**
 * Evalúa funciones de Calculator como SUM, AVERAGE, etc.
 */
function evaluateFunctions(expression, cells) {
    // Funciones soportadas
    const functions = {
        // Funciones de suma y promedio
        ...
        ABS: (values) => Math.abs(parseNumber(values[0])),
        ROUND: (values) => {
            const num = parseNumber(values[0]);
            const decimals = values[1] !== undefined ? parseNumber(values[1]) : 0;
            return Math.round(num * Math.pow(10, decimals)) / Math.pow(10, decimals);
        },
        FLOOR: (values) => Math.floor(parseNumber(values[0])),
        CEIL: (values) => Math.ceil(parseNumber(values[0])),
        SQRT: (values) => Math.sqrt(parseNumber(values[0])),
        POWER: (values) => Math.pow(parseNumber(values[0]), parseNumber(values[1])),
        ...
        IF: (values) => {
            ...
            // Nota: La condición debe estar pre-evaluada
            const condition = parseNumber(values[0]);
            return condition ? values[1] : (values[2] || '');
        },

        // Funciones estadísticas
        MEDIAN: (values) => {
            const nums = values.map(parseNumber).sort((a, b) => a - b);
            const mid = Math.floor(nums.length / 2);
            return nums.length % 2 === 0 ? (nums[mid - 1] + nums[mid]) / 2 : nums[mid];
        },
        PRODUCT: (values) => values.reduce((prod, val) => prod * parseNumber(val), 1),
    };
